...
class TigrisCallCredentialOauth2 extends CallCredentials {
  ...
  private TigrisCallCredentialOauth2(TigrisConfiguration tigrisConfiguration) {
    this.tokenService = new OAuth2TokenService(tigrisConfiguration.getoAuth2Config());
  }
   ...
   */
  static TigrisCallCredentialOauth2 getInstance(TigrisConfiguration configuration) {
    if (INSTANCE == null) {
      synchronized (lock) {
        if (INSTANCE == null) {
          INSTANCE = new TigrisCallCredentialOauth2(configuration);
        }
      }
    }
    return INSTANCE;
